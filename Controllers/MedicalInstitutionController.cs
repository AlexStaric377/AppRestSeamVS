...
namespace AppRestSeam.Controllers
{
    ...
    public class MedicalInstitutionController : ControllerBase
    {
        ...
        [HttpGet("{Edrpou}/{PostIndex}")]
        public async Task<ActionResult<MedicalInstitution>> Get(string Edrpou, string PostIndex)
        {

            if (Edrpou.Trim() == "0" && PostIndex.Trim() == "0") { return NotFound(); }
        ...
        public async Task<ActionResult<MedicalInstitution>> Delete(string id)
        {
            ...
            if (Convert.ToInt32(id) == -1)
            {
                ...
                {
                    _detailing = await db.MedicalInstitutions.FindAsync(Convert.ToInt32(str.Id));
                    if (_detailing != null)
                    {
            ...
            { 
                _detailing = await db.MedicalInstitutions.FindAsync(Convert.ToInt32(id));
